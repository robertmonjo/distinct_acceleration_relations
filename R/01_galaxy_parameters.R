...

{
  ...
  ##gamempty = seq_gammagc[order(apply(gal_xi22[i,,],2,min))[1]]
...
plot.new()
legend("bottom",legend=c("Observations","1-parameter model","2-parameter model"),ncol=3,col=c("black","red","blue"),lwd=c(0,1,1),lty=c(0,1,1), pch=20,pt.cex=c(1,0,0),bty="n")
...
abline(0,1)
plot(sqrt(smcGa07.mass/(4/3*pi*(smcGa07.R$x*kpc)^3)/(3/(8*pi*GN*T0^2)))/8,galeps_k)
abline(0,1)
...
#### Fig00C_interpolation.pdf (accel) ####
{
  ...
  
  {
    ...
    par(fig=c(0,1,0,0.2),new=TRUE,mar=c(0,0,0,0),oma=c(0,4,3,4))
    plot.new()
    legend("bottom",legend=c("1-parameter model","2-parameter model"),ncol=3,col=c("red","blue"),lwd=c(1,1),lty=c(1,1), pch=20,pt.cex=c(1,0,0),bty="n")
